...
class NepaliDateConverter
{
    private $bs = [
        ...
        84 => [2084, 31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30],
        ...
        92 => [2092, 31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31],
        93 => [2093, 31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 29, 31],
        94 => [2094, 31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30],
        95 => [2095, 31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30],
    ];
    ...
    public array $bsMonths = ['वैशाख', 'जेठ', 'असार', 'साउन', 'भदौ', 'असोज', 'कात्तिक', 'मङ्सिर', 'पुस', 'माघ', 'फागुन', 'चैत'];

    public array $enBsMonths = ['Baishakh', 'Jestha', 'Ashad', 'Shrawan', 'Bhadra', 'Ashwin', 'Kartik', 'Mangshir', 'Poush', 'Magh', 'Falgun', 'Chaitra'];
     ...
     * @param  int  $year
     * @param  int  $month
     * @return int Total days in a month of a given year
     */
     ...
     *
     * @param  int  $year
     * @return int total days in a year
     */
     ...
     * @param  Date  $end_date
     * @return int total number of days between start and end date
     */
    public function days_between($start_date, $end_date)
    {
        @[$sYear, $month, $day] = explode('-', $start_date);
        if ($this->is_range_nep($sYear, $month, $day) === false) {
            throw new Exception('Date out of range');
        }

        @[$eYear, $month, $day] = explode('-', $end_date);
    ...
    /**
     * Display nepali date in Y-m-d format
     *
     * @return string
     */
    public function convertToNepaliDate($date)
    {
        ...
        $nepaliDateArray = $this->eng_to_nep($year, $month, $day);

        $nMonth = $nepaliDateArray['month'];
        $nDay = $nepaliDateArray['date'];
        ...
        return $nepaliDateArray['year'].'-'.$nMonth.'-'.$nDay;
    }
    ...
    public function convertToEnglishDate($date)
    {
        ...
        $dateArray = $this->nep_to_eng($year, $month, $day);

        $dateAd = $dateArray['year'].'-'.$dateArray['month'].'-'.$dateArray['date'];

        return Carbon::createFromDate($dateAd)->format('l, j F Y');
    }
    ...
    */
    ...
    public function getBsMonth($num)
    {
        $bar = $this->bsMonths;
        $ret = $bar[$num - 1];
    ...
    {
        $bar = ['आइतबार', 'सोमबार', 'मङ्गलबार', 'बुधबार', 'बिहीबार', 'शुक्रबार', 'शनिबार'];
        $ret = $bar[$num - 1];
    ...
    private function englishToNepaliDate($dateArray)
    {
        ...
        $nepaliDate['year_ne'] = $this->getNepaliNumber($convertedDate['year']);
        $nepaliDate['year'] = $convertedDate['year'];
        $nepaliDate['month_name'] = $this->getBsMonth($convertedDate['month']);
        $nepaliDate['month_ne'] = $this->getNepaliNumber($convertedDate['month']);
        $nepaliDate['month'] = $convertedDate['month'];
        $nepaliDate['day'] = $this->getBaar($convertedDate['num_day']);
        $nepaliDate['date_ne'] = $this->getNepaliNumber($convertedDate['date']);
        $nepaliDate['date'] = $convertedDate['date'];
