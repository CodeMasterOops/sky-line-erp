...
export const useVendorStockStore = defineStore('vendor-stock', {
    state: () => ({
        stocks: {
            data: [],
            meta: {},
    ...
    actions: {
        getStockList({filter}) {
            ...
            return apiVendor(`${apiUrl}/list?${new URLSearchParams(filter).toString()}`)
                .then((res) => {
        ...
        updateStock(id, form) {
            return apiVendor(`${apiUrl}/${id}/update`, 'post', form)
                .then((res) => {
                    return res;
                }).catch((err) => {
                    throw err;
        ...
        getStockHistories({variant_id,filter}) {
            ...
            return apiVendor(`${apiUrl}/${variant_id}/history?${new URLSearchParams(filter).toString()}`)
                .then((res) => {
