  ...
  <section class="section">
    <div class="card">
      <div class="card-header d-flex justify-content-between">
        ...
        <router-link :to="{name:'vendor.product-list'}" class="btn btn-sm btn-outline-primary">
      ...
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-10">
            <form @submit.prevent="updateProduct(product.data.id)" class="border rounded-1 p-3">
                  ...
                  <div class="row g-3">
                    <div class="col-md-12">
                      <VFileUpload
                          ...
                          :default-photo="product.data.thumbnail_image_url"
                                ...
                                <div class="row g-3">
                                  <div class="col-md-6">
                                    <VFileUpload
                                        ...
                                        label="Image"
                                        :default-photo="product.data.images[index]?.image_url || ''"
              ...
              <div class="col-12 text-end mt-3">
                <router-link :to="{name:'vendor.product-list'}" class="btn btn-danger">
                  Close
                </router-link>
...
import {useVendorProductStore} from "@/stores/vendor/product.js";

const router = useRouter();
const route = useRoute();
const productStore = useVendorProductStore();
const settingStore = useVendorSettingStore();

onMounted(() => {
  settingStore.getProductCategories();
  settingStore.getTags();
  settingStore.getProductAttributes();
  settingStore.getBrands();
  settingStore.getBrands();
  setFormData();
})

const {productCategories, tags, productAttributes: attributes, brands} = storeToRefs(settingStore);
...
const setFormData = async () => {
  ...
  Object.keys(form).forEach(key => {
    if (key === 'categories') {
      form.categories = product.value.data.categories.map(a => a.id);
    } else if (key === 'tags') {
      form.tags = product.value.data.tags.map(t => t.id);
    } else if (key === 'images') {
      product.value.data.images.forEach(p => {
        form.images.push({
          id: p.id,
          title: p.title,
          image: p.image,
      ...
      product.value.data.variants.forEach(v => {
        form.variants.push({
          id: v.id,
          sku: v.sku,
          price: v.price,
      ...
      form.attribute_values = product.value.data.attribute_value_ids || [];
    } else if (key === 'is_active') {
      form.is_active = product.value.data.is_active ? 1 : 0;
    } else if (key === 'has_variants') {
      form.has_variants = product.value.data.has_variants ? 1 : 0;
    } else {
      form[key] = product.value.data[key] || ''
    }
  })
  product.value.data.attributes.forEach(attr => {
    selectedAttributeOptions.value.push({
...
const updateProduct = async (id) => {
  ...
  if (validated) {
    ...
    try {
      ...
      resetForm();
      await router.push({name: 'vendor.product-list'})
    } catch (e) {
      showErrors(e);
    } finally {
