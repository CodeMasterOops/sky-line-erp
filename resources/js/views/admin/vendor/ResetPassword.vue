<template>
  <VModal
      :show-modal="!!reset_vendor_password"
      @close-click="closeEditModal"
      title="Reset Password">
    <template #modal-body>
      <form @submit.prevent="resetPassword(reset_vendor_password)" class="row g-3">
        <div class="col-md-6">
          <VInput
              id="password"
...
const reset_vendor_password = defineModel('reset_password');
...
const resetPassword = async (id) => {
  ...
  if (validated) {
    ...
    try {
      let res = await vendorStore.resetPassword(id, form);
