<template>
    ...
    <section class="section">
        <div class="card border-0 shadow-sm">
            <VLoader v-if="role.loading || groups.loading" loader-type="progress"/>

            <div class="card-body p-4">
                <form @submit.prevent="updateRole(role.data.id)" class="row g-4">
                    <div class="col-12">
                        <VInput
                            id="name"
                            v-model="form.name"
...
onMounted(async () => {
    ...
    Object.keys(form).forEach(key => {
        form[key] = role.value.data[key] ?? initialState[key];
    });
});
...
const updateRole = async (id) => {
    ...
    try {
        const res = await roleStore.updateRole(id, form);
