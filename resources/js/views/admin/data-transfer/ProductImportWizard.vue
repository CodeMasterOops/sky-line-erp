<template>
    <div class="modal fade" id="product-import-wizard" tabindex="-1" aria-hidden="true" ref="modalEl">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div v-else-if="step === 1 && job">
                        <p class="text-muted small">Map file columns to product fields.</p>
                        <div class="mb-3">
                                    ...
                                    <tr v-for="header in detectedHeaders" :key="header">
                                        ...
                                        <td>
                                            <select v-model="mapping[header]" class="form-select form-select-sm">
                                                ...
                                                <option v-for="f in productFields" :key="f" :value="f">{{ f }}</option>
                                            </select>
                                        </td>
                                    </tr>
                                </tbody>
...
const downloadTemplate = (format) => store.downloadProductTemplate(format);

const upload = async () => {
    if (!selectedFile.value) return;
    uploading.value = true;
    try {
        const data = await store.uploadImport(selectedFile.value, 'product');
        job.value = data;
