<template>
    <PageHeader title="Journal Vouchers" subtitle="Manage your journal vouchers" @refresh="fetchVouchers">
        <template #actions>
            <button
                v-can="'create_journal_voucher'"
                type="button"
    ...
    <CreateJournalVoucher v-model:create-modal-opened="createModalOpened"/>
    <EditJournalVoucher v-model:voucher_id="edit_voucher_id"/>
    <JournalVoucherDetailModal v-model:detail-voucher-id="detail_voucher_id" />
</template>
...
import JournalVoucherDetailModal from './DetailModal.vue';
...
const {vouchers} = storeToRefs(journalVoucherStore);
...
const fetchVouchers = () => {
    journalVoucherStore.getVouchers({filter});
};
...
const deleteVoucher = async (id) => {
    Swal.fire({
        ...
        if (result.value) {
            try {
                let res = await journalVoucherStore.deleteVoucher(id);
...
const approveVoucher = async (id) => {
    Swal.fire({
        ...
        if (result.value) {
            try {
                let res = await journalVoucherStore.approveVoucher(id);
                toast(res.status, res.data.message);
                fetchVouchers();
            } catch (e) {
                showErrors(e);
