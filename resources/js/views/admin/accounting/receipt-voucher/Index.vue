<template>
    <PageHeader title="Receipt Vouchers" subtitle="Manage your receipt vouchers" @refresh="fetchVouchers">
        <template #actions>
            <button
                v-can="'create_receipt_voucher'"
                type="button"
...
const {vouchers} = storeToRefs(receiptVoucherStore);
...
const fetchVouchers = () => {
    receiptVoucherStore.getVouchers({filter});
};
...
const deleteVoucher = async (id) => {
    Swal.fire({
        ...
        if (result.value) {
            try {
                let res = await receiptVoucherStore.deleteVoucher(id);
...
const approveVoucher = async (id) => {
    Swal.fire({
        ...
        if (result.value) {
            try {
                let res = await receiptVoucherStore.approveVoucher(id);
                toast(res.status, res.data.message);
                fetchVouchers();
            } catch (e) {
                showErrors(e);
