<template>
    <PageHeader title="Currencies" subtitle="Manage foreign exchange rates (Base: NPR)">
        <template #actions>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr v-for="cur in currencies" :key="cur.id">
                            ...
                            <td><span class="badge" :class="cur.is_base ? 'bg-primary' : 'bg-light text-dark'">{{ cur.is_base ? 'Base' : '' }}</span></td>
                            <td><span class="badge" :class="cur.is_active ? 'bg-success' : 'bg-secondary'">{{ cur.is_active ? 'Active' : 'Inactive' }}</span></td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="btn btn-sm btn-outline-primary" @click="editCurrency(cur)"><i class="ti ti-edit"></i></button>
                                    <button class="btn btn-sm btn-danger" @click="deleteCurrency(cur.id)"><i class="ti ti-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3" v-if="!editingCurrency">
                        ...
                        <input type="text" class="form-control" v-model="form.code" maxlength="3" placeholder="USD, INR, EUR..." />
                    </div>
                    <div class="mb-3">
...
const saveCurrency = async () => {
    ...
    try {
        if (editingCurrency.value) {
            ...
            await apiAdmin('currency', 'post', form.value);
...
const deleteCurrency = async (id) => {
    const result = await Swal.fire({ title: 'Delete currency?', icon: 'warning', showCancelButton: true, confirmButtonColor: 'red', confirmButtonText: 'Yes' });
