<template>
    <PageHeader hide-action-buttons title="Daily Collection Report" subtitle="Total receipts collected per day" />
    ...
    <section class="section">
                        ...
                        <table class="table align-middle mb-0">
                            ...
                            <tbody>
                                <tr v-for="day in dailySummary" :key="day.date">
                                    ...
                                    <td class="text-end fw-semibold text-success">{{ formatMoney(day.total) }}</td>
                                </tr>
                            </tbody>
                            <tfoot class="table-light fw-semibold">
                                <tr>
                                    ...
                                    <td class="text-end text-success">{{ formatMoney(grandTotal) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                        ...
                        <table class="table table-sm align-middle mb-0">
                            ...
                            <tbody>
                                <tr v-for="(row, i) in allRows" :key="row.id">
                                    ...
                                    <td><span class="badge bg-success-subtle text-success">{{ row.type_label }}</span></td>
                                    <td class="text-end fw-semibold">{{ formatMoney(row.total_dr) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        ...
        <div v-else-if="!loading" class="card border-0 shadow-sm">
            <div class="card-body text-center text-muted py-5">
                <i class="ti ti-cash display-4 d-block mb-3"></i>
                Select a date range and click 'Generate' to view daily collections.
            </div>
        </div>
...
const dailySummary = computed(() => {
    ...
    allRows.value.forEach((row) => {
        ...
        map[row.date].count++;
        map[row.date].total += Number(row.total_dr ?? 0);
    });
...
const loadReport = async () => {
    loading.value = true;
    try {
        const params = {journal_type: 'receipt-voucher'};
        ...
        allRows.value = (data?.rows ?? []).map((row) => ({
            ...row,
            total_dr: row.items?.reduce((s, item) => s + Number(item.dr_amount ?? 0), 0) ?? 0,
        }));
    } catch (e) {
        showErrors(e);
    } finally {
...
const exportCsv = () => {
    ...
    const headers = ['Date', 'Vouchers', 'Total Collected'];
