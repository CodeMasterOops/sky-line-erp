<template>
    <PageHeader hide-action-buttons title="Monthly Purchase Report" subtitle="Purchase totals grouped by month" />
...
const loadReport = async () => {
    ...
    try {
        ...
        const res = await apiAdmin('purchase-report/monthly-purchase', 'get', params);
...
const exportCsv = () => {
    ...
    const headers = ['Month', 'Bills', 'Subtotal', 'Discount', 'Tax', 'Net Purchases'];
    const data = rows.value.map(r => [r.month_label, r.bill_count, r.subtotal, r.total_discount, r.tax_amount, r.net_purchases]);
    const csv = [headers, ...data].map(r => r.map(v => `"${v ?? ''}"`).join(',')).join('\n');
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([csv], {type: 'text/csv'}));
    a.download = 'monthly-purchase.csv';
    a.click();
};
