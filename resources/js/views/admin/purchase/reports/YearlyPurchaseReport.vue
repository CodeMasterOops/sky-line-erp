<template>
    <PageHeader hide-action-buttons title="Yearly Purchase Report" subtitle="Purchase totals grouped by year" />
...
const kpi = computed(() => reportData.value?.summary ?? {bill_count: 0, subtotal: 0, total_discount: 0, tax_amount: 0, net_purchases: 0});
...
const loadReport = async () => {
    ...
    try {
        ...
        const res = await apiAdmin('purchase-report/yearly-purchase', 'get', params);
...
const exportCsv = () => {
    ...
    const headers = ['Year', 'Bills', 'Subtotal', 'Discount', 'Tax', 'Net Purchases'];
    const data = rows.value.map(r => [r.bill_year, r.bill_count, r.subtotal, r.total_discount, r.tax_amount, r.net_purchases]);
    const csv = [headers, ...data].map(r => r.map(v => `"${v ?? ''}"`).join(',')).join('\n');
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([csv], {type: 'text/csv'}));
    a.download = 'yearly-purchase.csv';
    a.click();
};
...
onMounted(async () => {
    ...
    if (fy?.start_date && fy?.end_date) {
        ...
        const now = new Date();
        filters.value.from_date = `${now.getFullYear() - 4}-01-01`;
