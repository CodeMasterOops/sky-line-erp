...
watch(() => edit_order_id.value, async (id) => {
    if (id) {
        await purchaseOrderStore.getOrder(id);
        Object.keys(form).forEach(key => {
            if (key === 'items') {
                form.items = (order.value.data.items || []).map(item => ({
...
const updateOrder = async (id) => {
    ...
    if (validated) {
        ...
        try {
            syncTaxAmounts();
            let res = await purchaseOrderStore.updateOrder(id, form);
