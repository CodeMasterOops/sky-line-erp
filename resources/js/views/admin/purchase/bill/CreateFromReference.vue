...
const loadReference = async () => {
    ...
    loading.value = true;
    try {
        const endpoint = `purchase-order/${purchaseOrderId.value}`;
...
watch(() => [purchaseOrderId.value, open.value], ([id, isOpen]) => {
    if (id && isOpen) {
        loadReference();
    }
});

const storeBill = async (status = 'draft') => {
    form.status = status;
