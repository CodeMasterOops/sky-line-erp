    ...
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3">
      <div class="search-set">
        <div class="search-input">
          ...
          <input type="search" v-model="filter.search" class="form-control form-control-sm" placeholder="Search" @input="debouncedFetch">
        </div>
      </div>
      <div class="d-flex table-dropdown my-xl-auto right-content align-items-center flex-wrap row-gap-3">
        ...
        <div class="dropdown me-2">
          ...
          <ul class="dropdown-menu dropdown-menu-end p-3">
            <li>
               <a href="javascript:void(0);" class="dropdown-item rounded-1" @click="setFilter('product_category_id', '')">All Categories</a>
            </li>
            <li v-for="category in categoryList" :key="category.id">
              <a href="javascript:void(0);" class="dropdown-item rounded-1" @click="setFilter('product_category_id', category.id, category.name)">{{ category.name }}</a>
            </li>
          </ul>
        </div>
        <!-- Brand Dropdown -->
        <div class="dropdown me-2">
          ...
          <ul class="dropdown-menu dropdown-menu-end p-3">
            <li>
               <a href="javascript:void(0);" class="dropdown-item rounded-1" @click="setFilter('brand_id', '')">All Brands</a>
            </li>
            <li v-for="brand in brands" :key="brand.id">
              <a href="javascript:void(0);" class="dropdown-item rounded-1" @click="setFilter('brand_id', brand.id, brand.name)">{{ brand.name }}</a>
            </li>
          </ul>
        </div>
      </div>
            ...
            <template v-if="column.key === 'Product'">
              <div class="productimgname">
                <a href="javascript:void(0);" class="avatar avatar-md me-2">
                   <img :src="record.image || '/assets/img/product-placeholder.png'" alt="product" style="width: 40px; height: 40px; object-fit: cover;">
                </a>
            ...
            <template v-else-if="column.key === 'CreatedBy'">
              <div class="userimgname">
                <span class="avatar avatar-sm me-2">
                  <!-- user image placeholder since backend doesn't return user yet -->
                   <img src="@/assets/images/profiles/avatar-01.jpg" alt="user" style="width: 30px; height: 30px; border-radius: 50%;">
                </span>
                  <a href="javascript:void(0);">{{ record.created_by_name || 'Admin' }}</a>
              </div>
            </template>
            ...
            <template v-else-if="column.key === 'action'">
              <div class="action-table-data">
                <div class="edit-delete-action">
                  <a class="me-2 edit-icon p-2" href="javascript:void(0);" @click="editProduct(record.id)">
                    <i class="ti ti-edit"></i>
                  </a>
                  <a data-bs-toggle="modal" class="p-2" href="javascript:void(0);" @click="deleteProduct(record.id)">
