<template>
    <PageHeader title="Receipts" subtitle="Manage customer receipts" @refresh="fetchReceipts">
        <template #actions>
            <button v-can="'create_receipt'" type="button" @click.prevent="createModalOpened = true"
    ...
    <div class="card table-list-card">
        <VTableToolbar v-model="filter.search" placeholder="Search receipt" :is-filtered="isFiltered"
        ...
        <div class="card-body">
            <div class="custom-datatable-filter table-responsive">
                <a-table class="table datanew table-hover table-center mb-0" :columns="receiptColumns"
                    :data-source="receipts.data" :pagination="false" :loading="receipts.loading"
                    @change="handleTableChange">
                    <template #bodyCell="{ column, record, index }">
                        <template v-if="column.key === 'sn'">
                            {{ (receipts.meta.from || ((filter.page - 1) * filter.limit + 1)) + index }}
                        </template>
                ...
                <VPagination v-model:page="filter.page" v-model:limit="filter.limit" :meta="receipts.meta" />
            </div>
        </div>
    </div>
...
import { receiptColumns, createRowActions } from './tableConfig.js';
...
const { handleTableChange } = useTablePagination({
    meta: computed(() => receipts.value.meta),
    filter,
});
...
const handleApprove = (id) => {
    confirmAction({
        ...
        action: () => receiptStore.approveReceipt(id),
