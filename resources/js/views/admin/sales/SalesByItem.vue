<template>
    <PageHeader hide-action-buttons title="Sales By Item" subtitle="Sales report" @refresh="generateReport" />
    ...
    <section class="section">
                ...
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-xl-2 col-lg-3">
                        <button
                            ...
                            :disabled="salesByItem.loading || !filter.from_date || !filter.to_date"
                            @click="generateReport"
                        >
                            {{ salesByItem.loading ? 'Generating...' : 'Generate' }}
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="card sales-by-item-card border-0 shadow-sm">
            <div class="card-body p-0">
                <div class="sales-by-item-header">
                    <div>
                        <h4 class="mb-1">Sales By Item</h4>
                ...
                <div class="table-responsive">
                    <table class="table sales-by-item-table align-middle mb-0">
                        <thead>
                            <tr>
                                <th>Product</th>
                        ...
                        <tbody>
                            <VLoader v-if="salesByItem.loading" :colspan="7" />
                            ...
                            <tr v-else>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    No sales-by-item data found for the selected filters.
                                </td>
                            </tr>
                        </tbody>
                        <tfoot v-if="!salesByItem.loading">
...
const reportRows = computed(() => salesByItem.value.data?.rows || []);
const productVariantOptions = computed(() => salesByItem.value.data?.product_variant_options || []);
const summary = computed(() => salesByItem.value.data?.summary || {});
const reportPeriodLabel = computed(() => {
    const period = salesByItem.value.data?.period;
...
const generateReport = async () => {
    await salesReportStore.getSalesByItem({
...
.sales-by-item-table tbody td,
.sales-by-item-table tfoot th {
    border-color: #e7edf5;
}
