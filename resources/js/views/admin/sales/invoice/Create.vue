...
function commitVariantLine(variant, warehouse) {
    ...
    form.items.push({
        ...
        tax_id: variant.tax_group_id ? '' : (variant.tax_id ? String(variant.tax_id) : ''),
        tax_group_id: variant.tax_group_id || null,
        tax_amount: 0,
        line_discount_type: 'fixed',
        line_discount_value: '0',
...
const onLineTaxChange = async (index, value) => {
    if (value && String(value).startsWith('group:')) {
        ...
        // Fetch server-side computed amount for inclusive/compound accuracy.
        try {
