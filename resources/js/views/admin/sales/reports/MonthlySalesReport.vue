<template>
    <PageHeader hide-action-buttons title="Monthly Sales Report" subtitle="Invoice totals grouped by month showing trends across the fiscal year" />
                    ...
                    <table class="table table-sm table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(row, idx) in rows" :key="idx">
                                <td class="fw-semibold text-nowrap">{{ row.month_label }}</td>
...
const loadReport = async () => {
    ...
    try {
        ...
        const res = await apiAdmin('sales-report/monthly-sales', 'get', params);
...
const exportCsv = () => {
    ...
    const headers = ['Month', 'Invoices', 'Subtotal', 'Discount', 'Tax', 'Net Sales'];
    const csvRows = rows.value.map(r => [r.month_label, r.invoice_count, r.subtotal, r.discount, r.tax_amount, r.net_sales].map(v => `"${v ?? ''}"`).join(','));
