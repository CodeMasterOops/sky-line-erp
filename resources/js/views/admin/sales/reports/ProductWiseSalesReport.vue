<template>
    <PageHeader hide-action-buttons title="Product Wise Sales Report" subtitle="Sales grouped by product showing quantity sold and revenue contribution" />
                        ...
                        <tbody>
                            ...
                            <tr v-for="(row, idx) in rows" :key="row.product_id">
                                ...
                                <td>
                                    ...
                                    <small v-if="row.product_code" class="text-muted d-block">{{ row.product_code }}</small>
                                </td>
                                <td>{{ row.category_name }}</td>
                                <td class="text-end">{{ row.invoice_count }}</td>
                                <td class="text-end fw-semibold">{{ formatMoneyPlain(row.total_qty) }}</td>
...
const filters = ref({from_date: '', to_date: '', category_id: '', party_id: ''});
...
const loadReport = async () => {
    ...
    try {
        ...
        const res = await apiAdmin('sales-report/product-wise-sales', 'get', params);
...
const exportCsv = () => {
    ...
    const headers = ['#', 'Product', 'Code', 'Category', 'Invoices', 'Qty Sold', 'Subtotal', 'Discount', 'Tax', 'Net Sales'];
    const csvRows = rows.value.map((r, i) => [
        i + 1, r.product_name, r.product_code, r.category_name,
        r.invoice_count, r.total_qty, r.subtotal, r.discount, r.tax_amount, r.net_sales,
    ].map((v) => `"${v ?? ''}"`).join(','));
