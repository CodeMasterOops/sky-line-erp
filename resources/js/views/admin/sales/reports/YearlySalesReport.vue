<template>
    <PageHeader hide-action-buttons title="Yearly Sales Report" subtitle="Invoice totals grouped by fiscal year for long-term trend analysis" />
                    ...
                    <table class="table table-sm table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(row, idx) in rows" :key="idx">
                                <td class="fw-semibold">{{ row.sale_year }}</td>
...
const loadReport = async () => {
    ...
    try {
        ...
        const res = await apiAdmin('sales-report/yearly-sales', 'get', params);
...
const exportCsv = () => {
    ...
    const headers = ['Year', 'Invoices', 'Subtotal', 'Discount', 'Tax', 'Net Sales'];
    const csvRows = rows.value.map(r => [r.sale_year, r.invoice_count, r.subtotal, r.discount, r.tax_amount, r.net_sales].map(v => `"${v ?? ''}"`).join(','));
    const csv = [headers.join(','), ...csvRows].join('\n');
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([csv], {type: 'text/csv'}));
    a.download = 'yearly-sales.csv';
    a.click();
};
